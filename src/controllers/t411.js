...
const torrentRouter = () => {
  ...
  const baseUrl = 'https://api.t411.al'
  ...
  function profileRefresh (req, res, next) {
    const user = db.get(req.user, {})
    const token = (user.t411 || {}).token || null
    const uid = (user.t411 || {}).uid || null
    if (!token || !uid) {
  ...
  router.get('/profile', (req, res, next) => {
    ...
    const token = (user.t411 || {}).token || null
    const uid = (user.t411 || {}).uid || null
    const profile = (user.t411 || {}).profile || null
  ...
  router.post('/search', (req, res, next) => {
    ...
    const token = (db.get(req.user, {}).t411 || {}).token || null
    if (!token) {
